...
class Model(nn.Module):
    ...
    def forward(self, position, direction):
        ...
        x = torch.relu(self.last_layer(x))
        # print(x.dtype)
...
def render_rays(model, frame):
    ...
    print(dt.shape, frame.rays_d.shape)
    dt = dt*torch.norm(directions[..., None, :], dim = -1)
    ...
    # print(alpha.shape)
    # print(color.shape)
    rgb = torch.sum(alpha[:,:,None] * transmittance[:,:,None] * color, dim=1)
    del dt
    return rgb
...
def train(model, epochs, data, learning_rate = 5e-4):
    ...
    index_test = np.random.randint(len(data.test_frames))
    # print(len(data.train_frames))
    plt.imshow(data.test_frames[index_test].img)
    plt.savefig(f'test_img_{index_test}_original.png')
    plt.close()
    # plt.savefig(f'drive/MyDrive/EECS 504 Final Project/NOVEL_NERF/Model/test_img_actual.png')
    # plt.close()
    data.test_frames[index_test].get_rays(device)
    sample_frame(data.test_frames[index_test], 155, 2, 6, dev="cuda:0")
    count = 0
    for fr in data.train_frames:
        # print("Getting rays and sampling")
        fr.get_rays(device)
        sample_frame(fr, 155, 2, 6, dev = 'cuda:0')
        # print(count)
        count +=1
    ls = []
    for i in range(epochs):
      ...
      # print(fr.img.device)
      loss = torch.nn.functional.mse_loss(predicted.reshape([actual.shape[0], actual.shape[1], actual.shape[2]-1]), actual[:, :, :-1]) # fr.img is (800,800,4)
...
def test(model, data, index_test, epoch, loss):
    model.eval()
    fr = data.test_frames[index_test]
    # img = fr.img.to(device)
    # fr.get_rays(device)
    # sample_frame(fr, 150, 2, 8, dev = 'cuda:0')
    predicted = render_rays(model, fr)
...
if __name__ == '__main__':
    ...
    frameManager.read_frames(DATADIR)
    gc.collect()
    torch.cuda.empty_cache()
    model = Model()
    finer_model = Model()
